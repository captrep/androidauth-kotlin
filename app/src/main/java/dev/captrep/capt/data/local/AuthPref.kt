package dev.captrep.capt.data.local
...
import dev.captrep.capt.util.getObject
import dev.captrep.capt.util.putObject
...
class AuthPref(val context:Context) {
    ...
    suspend fun login(email: String, password: String): ActionState<AuthUser>{
        val user = authUser
        return if (user == null){
            ActionState(message = "Anda belum terdaftar pada sistem kami!", isSuccess = false)
        }else if(email.isBlank() || password.isBlank()){
            ActionState(message = "Field email dan password tidak boleh kosong!", isSuccess = false)
    ...
    suspend fun register(user: AuthUser): ActionState<AuthUser>{
        return if (user.email.isBlank() || user.password.isBlank()){
            ...
            authUser = user
            ActionState(user, message = "Pendaftaran berhasil dilakukan!")
        }
    }

    suspend fun logout(): ActionState<Boolean>{
        isLogin = false
        return ActionState(true,"Anda berhasil logout!")
    }
}